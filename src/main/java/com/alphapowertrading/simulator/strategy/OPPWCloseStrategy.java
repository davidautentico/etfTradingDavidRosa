...
//@Component("oppw")
public class OPPWCloseStrategy implements Strategy {
    ...
    private void managePosition(
        ...
        if (candle.open() <= sl) {
            broker.sell(candle.date(), candle.close(), "OPEN<=SL " + dayOfWeek);
            return;
        }

        if (candle.open() >= tp) {
            broker.sell(candle.date(), candle.close(), "OPEN>=TP " + dayOfWeek);
            return;
        }
