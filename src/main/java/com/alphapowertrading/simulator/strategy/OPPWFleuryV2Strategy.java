...
public class OPPWFleuryV2Strategy implements Strategy {
    ...
    private void managePosition(
        ...
        double actualProfitPer =
                (double) (candle.open() - entry) / entry;
    ...
    private double calculateGap(MarketContext context) {
        ...
        Candle candle = context.candle();
        Candle previous =
                context.marketData().get(context.index() - 1);

        return (double) (candle.open() - previous.close())
                / previous.close();
    }

    private boolean shouldCloseByGap(
            double actualProfitPer,
            double gapPer) {

        return actualProfitPer < CURRENT_LOSS
                && gapPer >= openGap;
    }
    ...
    private boolean shouldCloseWeekly(
    ...
    private void buy(
        ...
        int shares =
                (int) (broker.cash() * allocation / price);
    ...
    private boolean isLastDayOfWeek(
        ...
        LocalDate current =
                LocalDate.from(marketData.get(index).date());

        LocalDate next =
                LocalDate.from(marketData.get(index + 1).date());
