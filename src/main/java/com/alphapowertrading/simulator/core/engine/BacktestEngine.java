...
public class BacktestEngine {
    ...
    public BacktestEngine(double initialCapital, boolean showTrades, boolean showLossWeek, double lossWeekThreshold, boolean showMaxDd) {
        this(initialCapital, showTrades, showLossWeek, lossWeekThreshold, showMaxDd, 0.0);
    }

    public BacktestEngine(double initialCapital, boolean showTrades, boolean showLossWeek, double lossWeekThreshold, boolean showMaxDd, double commissionRate) {
    ...
    public BacktestReport run(MarketData marketData, Strategy strategy) {
        if (marketData == null || marketData.size() == 0) throw new IllegalArgumentException("Market data is empty");

        Broker broker = new Broker(initialCapital, showTrades, commissionRate);
        Candle firstCandle = marketData.get(0);
        MarketState marketState = new MarketState(firstCandle.close(), firstCandle.date());
        strategy.initialize(broker, marketData);

        for (int i = 0; i < marketData.size(); i++) {
            Candle candle = marketData.get(i);
            marketState.update(candle.close(), candle.date());

            MarketContext context = new MarketContext(candle, marketData, i,
                    marketState.currentDrawdown(candle.close()),
                    marketState.currentLowDrawdown(candle.low()),
                    marketState.peakClose(), marketState.peakDate());
            ...
            if (showMaxDd && newMaxDd) printMaxDrawdown(candle, broker);
    ...
    private void checkClosedTrade(Broker broker, int tradesBefore, MarketData marketData) {
        if (!showLossWeek || broker.trades().size() <= tradesBefore) return;
        Trade trade = broker.trades().getLast();
        if (pnlPercentage(trade) <= lossWeekThreshold) printLossWeek(trade, marketData);
    }
    ...
    private void printMaxDrawdown(Candle candle, Broker broker) {
        ...
        System.out.printf("Date: %s | Low: %.2f | MaxDD: %.2f%%%n", candle.date(), candle.low() * 0.01, broker.maxDrawdown() * 100);
        System.out.printf("Peak Equity: %.2f | Equity at Low: %.2f%n", broker.peakEquity(), broker.equity(candle.low()));
        System.out.println("=======================================");
    }

    private void printLossWeek(Trade trade, MarketData marketData) {
        LocalDate date = trade.exitDate();
        LocalDate monday = date.with(TemporalAdjusters.previousOrSame(DayOfWeek.MONDAY));
        LocalDate friday = date.with(TemporalAdjusters.nextOrSame(DayOfWeek.FRIDAY));

        System.out.println();
        System.out.printf("LOSS WEEK | PnL: %.2f%% | Reason: %s%n", pnlPercentage(trade), trade.closeReason());
        System.out.printf("Trade: %s -> %s | Week: %s -> %s%n", trade.entryDate(), trade.exitDate(), monday, friday);
        System.out.printf("%-12s %10s %10s %10s %10s%n", "Date", "Open", "High", "Low", "Close");

        for (Candle candle : marketData.candles()) {
            if (!candle.date().isBefore(monday) && !candle.date().isAfter(friday)) {
                System.out.printf("%-12s %10.2f %10.2f %10.2f %10.2f%n",
                        candle.date(), candle.open() * 0.01, candle.high() * 0.01,
                        candle.low() * 0.01, candle.close() * 0.01);
            }
        }
